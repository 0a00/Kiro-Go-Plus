...
func TestRecordApiKeyUsageConcurrent(t *testing.T) {
	...
	if err := FlushPendingWrites(); err != nil {
		t.Fatalf("flush usage: %v", err)
	}
	if err := Init(cfgFile); err != nil {
		t.Fatalf("reload persisted usage: %v", err)
	}
	persisted := GetApiKeyEntry(created.ID)
	if persisted == nil || persisted.TokensUsed != expectedTokens || persisted.CreditsUsed != expectedCredits || persisted.RequestsCount != expectedRequests {
		t.Fatalf("persisted usage mismatch: %+v", persisted)
	}
}
