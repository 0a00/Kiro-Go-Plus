...
(() => {
  ...
  async function loadRetryConfig() {
    ...
    $('retryPreOutputStreamRetries').value = d.preOutputStreamRetries ?? 1;
    $('retryPreOutputRetryBackoffMs').value = d.preOutputRetryBackoffMs || 700;
  ...
  async function saveRetryConfig() {
    const body = {
      ...
      preOutputStreamRetries: Math.round(Number($('retryPreOutputStreamRetries').value) || 0),
      preOutputRetryBackoffMs: Math.round(Number($('retryPreOutputRetryBackoffMs').value) || 0),
