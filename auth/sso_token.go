...
func registerDeviceClient(oidcBase, startUrl string) (clientID, clientSecret string, err error) {
	...
	json.NewDecoder(httpbody.LimitReader(resp.Body, httpbody.DefaultLimit)).Decode(&result)
	...
	for {
		...
		case <-ticker.C:
			...
			if resp.StatusCode == 400 {
				...
				json.NewDecoder(httpbody.LimitReader(resp.Body, httpbody.DefaultLimit)).Decode(&errResult)
				resp.Body.Close()
