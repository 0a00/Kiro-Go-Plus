...
var ErrRefreshUpstreamBlocked = errors.New("authentication refresh upstream temporarily blocked")

func IsRefreshUpstreamBlocked(err error) bool {
	return errors.Is(err, ErrRefreshUpstreamBlocked)
}

func isCloudFrontBlockedResponse(statusCode int, body []byte) bool {
	if statusCode != http.StatusForbidden {
		return false
	}
	lower := strings.ToLower(string(body))
	return strings.Contains(lower, "generated by cloudfront") ||
		strings.Contains(lower, "request blocked") ||
		strings.Contains(lower, "the request could not be satisfied")
}
