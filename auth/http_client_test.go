...
func TestBuildAuthTransportUsesExplicitProxyURL(t *testing.T) {
	transport, err := buildAuthTransport("http://proxy.local:8080")
	if err != nil {
...
func TestBuildAuthTransportFallsBackToEnvironmentProxy(t *testing.T) {
	...
	transport, err := buildAuthTransport("")
	if err != nil {
		t.Fatalf("build transport: %v", err)
	}
	req := &http.Request{URL: mustParseURL(t, "https://oidc.us-east-1.amazonaws.com")}
...
func TestBuildAuthTransportRejectsMalformedProxy(t *testing.T) {
	if _, err := buildAuthTransport("socks5://proxy-without-port"); err == nil {
		t.Fatal("expected malformed auth proxy to be rejected")
	}
}
