...
func StartBuilderIdLogin(region string) (*BuilderIdSession, error) {
	...
	if regResp.StatusCode != 200 {
		respBody := httpbody.ReadAllTruncated(regResp.Body, httpbody.DefaultLimit)
	...
	if err := json.NewDecoder(httpbody.LimitReader(regResp.Body, httpbody.DefaultLimit)).Decode(&regResult); err != nil {
	...
	if authResp.StatusCode != 200 {
		respBody := httpbody.ReadAllTruncated(authResp.Body, httpbody.DefaultLimit)
	...
	if err := json.NewDecoder(httpbody.LimitReader(authResp.Body, httpbody.DefaultLimit)).Decode(&authResult); err != nil {
...
func PollBuilderIdAuth(sessionID string) (accessToken, refreshToken, clientID, clientSecret, region string, expiresIn int, status string, err error) {
	...
	if tokenResp.StatusCode == 200 {
		...
		if err := json.NewDecoder(httpbody.LimitReader(tokenResp.Body, httpbody.DefaultLimit)).Decode(&tokenResult); err != nil {
	...
	if tokenResp.StatusCode == 400 {
		...
		json.NewDecoder(httpbody.LimitReader(tokenResp.Body, httpbody.DefaultLimit)).Decode(&errResult)
