...
func TestConversationIDNamespaceSeparatesClientsAndRemainsStable(t *testing.T) {
	baseA := &KiroPayload{}
	baseA.ConversationState.ConversationID = "base-conversation"
	baseB := &KiroPayload{}
	baseB.ConversationState.ConversationID = "base-conversation"
	baseC := &KiroPayload{}
	baseC.ConversationState.ConversationID = "base-conversation"

	namespaceConversationID(baseA, "api-key:key-a")
	namespaceConversationID(baseB, "api-key:key-a")
	namespaceConversationID(baseC, "api-key:key-b")

	if baseA.ConversationState.ConversationID != baseB.ConversationState.ConversationID {
		t.Fatalf("same namespace must be stable: %q vs %q", baseA.ConversationState.ConversationID, baseB.ConversationState.ConversationID)
	}
	if baseA.ConversationState.ConversationID == baseC.ConversationState.ConversationID {
		t.Fatalf("different client namespaces must not share a conversation ID")
	}
}
...
func TestOpenAIToolNamesUseStableCollisionSafeMapping(t *testing.T) {
	...
	req := &OpenAIRequest{
		...
		Tools: []OpenAITool{
			mustTool(t, fmt.Sprintf(`{"type":"function","name":%q,"parameters":{"type":"object"}}`, first)),
			mustTool(t, fmt.Sprintf(`{"type":"function","name":%q,"parameters":{"type":"object"}}`, second)),
		},
	}
	...
	if ctx == nil || len(ctx.Tools) != 2 {
		t.Fatalf("expected two mapped tools, got %#v", ctx)
	}
	firstMapped := ctx.Tools[0].ToolSpecification.Name
	secondMapped := ctx.Tools[1].ToolSpecification.Name
	if firstMapped == secondMapped {
		t.Fatalf("long tool names collided at %q", firstMapped)
	}
	if len(firstMapped) > maxToolNameLen || len(secondMapped) > maxToolNameLen {
		t.Fatalf("mapped names exceed %d bytes: %q %q", maxToolNameLen, firstMapped, secondMapped)
	}
	if payload.ToolNameMap[firstMapped] != first || payload.ToolNameMap[secondMapped] != second {
		t.Fatalf("missing reverse mapping: %#v", payload.ToolNameMap)
	}
	lastHistory := payload.ConversationState.History[len(payload.ConversationState.History)-1]
	if lastHistory.AssistantResponseMessage == nil || len(lastHistory.AssistantResponseMessage.ToolUses) != 1 {
		t.Fatalf("expected active structured tool history, got %#v", lastHistory)
	}
	if got := lastHistory.AssistantResponseMessage.ToolUses[0].Name; got != firstMapped {
		t.Fatalf("history name %q does not match definition %q", got, firstMapped)
	}
}

func TestClaudeSanitizedToolNamesDoNotCollide(t *testing.T) {
	tools := []ClaudeTool{
		{Name: "mcp__alpha__read_file", InputSchema: map[string]interface{}{"type": "object"}},
		{Name: "mcp--alpha--read-file", InputSchema: map[string]interface{}{"type": "object"}},
	}
	converted, restore := convertClaudeTools(tools)
	if len(converted) != 2 {
		t.Fatalf("expected two tools, got %d", len(converted))
	}
	first := converted[0].ToolSpecification.Name
	second := converted[1].ToolSpecification.Name
	if first == second {
		t.Fatalf("sanitized names collided at %q", first)
	}
	if restore[first] != tools[0].Name || restore[second] != tools[1].Name {
		t.Fatalf("unexpected restore map: %#v", restore)
	}
}
