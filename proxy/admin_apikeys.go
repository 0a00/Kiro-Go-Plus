...
func (h *Handler) apiUpdateApiKey(w http.ResponseWriter, r *http.Request, id string) {
	...
	if req.TokensPerMinute != nil {
		patch.TokensPerMinute = *req.TokensPerMinute
	}
	if req.MaxConcurrency != nil {
		patch.MaxConcurrency = *req.MaxConcurrency
	}
	if req.QueueCapacity != nil {
		patch.QueueCapacity = *req.QueueCapacity
	}
	if req.QueueTimeoutMs != nil {
		patch.QueueTimeoutMs = *req.QueueTimeoutMs
	}
	if err := validateApiKeyAdmissionLimits(patch.RequestsPerMinute, patch.TokensPerMinute, patch.MaxConcurrency, patch.QueueCapacity, patch.QueueTimeoutMs); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": err.Error()})
		return
	}
...
func validateApiKeyAdmissionLimits(rpm int, tpm int64, concurrency, queueCapacity, queueTimeoutMs int) error {
	if rpm < 0 || rpm > 1_000_000 {
		return fmt.Errorf("requestsPerMinute must be between 0 and 1000000")
	}
	if tpm < 0 || tpm > 1_000_000_000_000 {
		return fmt.Errorf("tokensPerMinute must be between 0 and 1000000000000")
	}
	if concurrency < 0 || concurrency > 10_000 {
		return fmt.Errorf("maxConcurrency must be between 0 and 10000")
	}
	if queueCapacity < 0 || queueCapacity > 100_000 {
		return fmt.Errorf("queueCapacity must be between 0 and 100000")
	}
	if queueTimeoutMs < 0 || queueTimeoutMs > 60_000 {
		return fmt.Errorf("queueTimeoutMs must be between 0 and 60000")
	}
	return nil
}
