...
func convertResponsesInputItems(items []json.RawMessage) ([]OpenAIMessage, error) {
	...
	for index, item := range items {
		...
		if err := json.Unmarshal(item, &obj); err != nil {
			return nil, fmt.Errorf("input item %d is invalid JSON: %w", index, err)
		}
		if obj == nil {
		...
		case typ == "message" || (typ == "" && role != ""):
			...
			if msg == nil {
				return nil, fmt.Errorf("input item %d contains an invalid message", index)
			}
		...
		case typ == "function_call_output" || typ == "tool_result":
			...
			if strings.TrimSpace(callID) == "" {
				return nil, fmt.Errorf("input item %d function_call_output requires call_id", index)
			}
		...
		case typ == "function_call":
			...
			if strings.TrimSpace(tc.ID) == "" || strings.TrimSpace(tc.Function.Name) == "" {
				return nil, fmt.Errorf("input item %d function_call requires call_id and name", index)
			}
		...
		case typ == "input_text" || typ == "text":
			...
			if text == "" {
				return nil, fmt.Errorf("input item %d requires non-empty text", index)
			}
		...
		case typ == "output_text":
			...
			text, _ := obj["text"].(string)
			if text == "" {
				return nil, fmt.Errorf("input item %d requires non-empty output text", index)
			}
		...
		default:
			...
			return nil, fmt.Errorf("input item %d has unsupported type %q", index, typ)
		}
	}
