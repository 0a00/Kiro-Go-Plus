...
func TestAuthenticatePublicBindWithoutExplicitOptOutFailsClosed(t *testing.T) {
	mustInitConfig(t)
	t.Setenv("ALLOW_UNAUTHENTICATED_API", "")

	h := &Handler{}
	_, err := h.authenticate(newAuthTestRequest(t, "", ""))
	ae, ok := err.(*authError)
	if !ok || ae.status != http.StatusUnauthorized {
		t.Fatalf("expected public bind to fail closed, got %T %v", err, err)
	}
}
