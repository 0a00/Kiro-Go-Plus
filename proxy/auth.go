...
func (h *Handler) authenticate(r *http.Request) (*config.ApiKeyEntry, error) {
	if !publicAPIAuthenticationRequired() {
		return nil, nil
	}
...
func publicAPIAuthenticationRequired() bool {
	if config.IsApiKeyRequired() {
		return true
	}
	host := strings.ToLower(strings.TrimSpace(config.GetHost()))
	publicBind := host == "" || host == "0.0.0.0" || host == "::" || host == "[::]"
	if !publicBind {
		return false
	}
	return !strings.EqualFold(strings.TrimSpace(os.Getenv("ALLOW_UNAUTHENTICATED_API")), "true")
}
